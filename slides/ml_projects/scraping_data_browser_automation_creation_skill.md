...
Use the `cloakbrowser` package (anti-detection wrapper around Patchright/Playwright). Install: `pip install cloakbrowser`.
...
### CloakBrowser development patterns

#### Persistent profile for session reuse
Store browser profile in a local directory. Cookies and session state survive restarts — avoids re-login on every run.
```python
profile_dir = os.path.join(os.path.dirname(__file__), "profile")
context = launch_persistent_context(profile_dir, headless=False, humanize=True)
```

#### Clipboard-paste for sensitive fields (anti-detection)
Do NOT use `.fill()` for login credentials — bot-detection scripts watch for programmatic input. Instead, copy to clipboard and paste via keyboard shortcut:
```python
import subprocess
...
time.sleep(1)
```
Use `.fill()` normally for non-sensitive form fields (names, dates, dropdowns).

#### Session expiry detection via URL redirect
After navigating to a protected page, check if the site redirected to a login/home page:
```python
page.goto("https://example.com/Services")
time.sleep(3)

if "example.com/Home" in page.url:
    logger.info("Session expired — re-logging in...")
    # perform login sequence
```

#### Manual login fallback
When automated login fails (CAPTCHA, 2FA, layout change), fall back to manual intervention. Use `input()` to pause the script — but ONLY during the login phase, never inside the polling loop:
```python
try:
    ...
    logger.warning("[MANUAL LOGIN REQUIRED] Please log in manually.")
    input("Press Enter after you have logged in... ")
```

#### Page reuse from existing context
A persistent context may already have open pages from a previous run. Reuse them instead of always creating new ones:
```python
page = context.pages[0] if context.pages else context.new_page()
```
