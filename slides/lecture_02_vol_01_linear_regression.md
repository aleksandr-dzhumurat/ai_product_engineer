...
Модель линейной регрессии сводится к нахождению коэффициентов $w_1,\ldots,w_n$:

$$
\forall x_i: h(x) = w_0 + w_1x_i^1 + \ldots + w_nx_i^n = \sum_{j=1}^{n}w_jx_i^j = \overline{x}_i^T\overline{w}
$$

Предсказание алгоритма - линейная комбинация от признаков.
...
| Сценарий | κappa | Значение |
| --- | --- | --- |
| Независимые признаки | ~1–100 | Стабильное решение |
| Умеренная корреляция | 100–1 000 | Начинается нестабильность |
| area_m2 + area_ft2 | greater 100000 | Коэффициенты "взрываются" |
| Полный дубликат признака | ∞ (сингулярность) | Решения не существует |
...
Почему в sklearn нет реализации MAPE loss для линейной регрессии? Философия sklearn: Sklearn включает методы, которые:
...
`from sklearn.linear_model import QuantileRegressor` (Это аппроксимация по сути)
