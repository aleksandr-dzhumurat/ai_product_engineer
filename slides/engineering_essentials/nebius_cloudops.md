...
# Nebius AI CloudOps Engineer — Concept Study Guide

> This is a study guide built around the themes that recurred across the questions we discussed.
> It gives concept understanding and reasoning patterns, not a "question → correct option" answer key.
> Where marked ⚠️ — verify exact flag/command names against the official Nebius documentation (CLI/Terraform) rather than memorizing an approximation.

---

## 1. Slurm operations (Soperator)

### Job lifecycle — the three core verbs
- **Observe:** `squeue -u $USER` — lists your jobs, their state (`R` = running, `PD` = pending), and the node column (NODELIST), i.e. which workers the job landed on.
- **Watch live output:** `tail -f` on the file Slurm writes stdout to. The filename is set by the `#SBATCH --output=` directive in the batch script; tokens like `%x` (job name) and `%j` (job ID) expand into the final name.
- **Cancel:** `scancel <jobid>`.

### Common distractor traps
- `queue` — not a Slurm command (correct is `squeue`). A wrong option often gives itself away in its very first token.
- `sbatch --cancel` — confuses submitting a job with cancelling one. `sbatch` only *submits*.
- `salloc` — interactive resource allocation, not monitoring or cancelling.
- `journalctl -u slurmd -f` — system logs for the Slurm daemon, not your job's stdout.
- The filename in `tail -f` must match the pattern from `--output`, not some arbitrary name from the prompt.

### Reasoning pattern
Map each sub-task to its verb: `squeue` to observe, `tail -f` on the `--output` file to watch, `scancel` to kill. Reject any sequence containing a non-existent command or one from a different lifecycle phase.

---

## 2. Slurm node states — `drain`

- `drain` — the node is removed from scheduling for **new** jobs (typically a health-check flag or maintenance). Jobs already running on it are allowed to finish.
- `drain` is about **scheduling availability**, not billing and not a cluster-wide fault.
- A node in `drain` has **no effect** on jobs running on other nodes.

### Traps
- Framing `drain` as a "billing state."
- Claiming "the only fix is to delete and recreate the cluster."
- Treating one degraded node as a threat to jobs on other nodes.

### Reasoning pattern
Separate three things: *scheduling availability* (will the node accept new work?), *job health* (is already-running work affected?), and *billing*. `drain` touches only the first. The correct action is not to panic or "recreate the cluster" but to **investigate the cause** of the drain before reusing the node. The exam loves options that inflate a benign state into a dramatic "fix."

---

## 3. Cluster storage: `/mnt/shm` vs the shared filesystem

- `/mnt/shm` — shared memory, backed by RAM on a **single node** (tmpfs). Two consequences: it is **not shared** across nodes and **not retained** after node restart / job end. It's fast precisely because it's RAM and the data never leaves the node.
- The shared (networked) filesystem is mounted on all login and worker nodes: a path written in one place is visible everywhere, and data survives across jobs.

### Symptom of the problem
A job fails with "no such file or directory" **only after moving nodes** → almost certainly you wrote to node-local storage (`/mnt/shm`) and are reading where the scheduler guaranteed nothing.

### Correct model
Durable, shared assets (scripts, Python venv) → shared FS. Ephemeral per-job scratch → `/mnt/shm`. In the case we discussed, the engineer did the reverse.

> Also: a Python venv travels poorly even across a shared FS because of absolute paths baked into shebangs and `pyvenv.cfg`.

---

## 4. Networking model — Virtual Networks (VPC)

### VPC as the umbrella service
Virtual Networks is a service that provides **all the networking primitives** that Nebius resources plug into: networks, subnets, pools, routes, security groups (plus allocations, routing tables). Its purpose comes down to two things:
- **connectivity** (IP connectivity between resources within a network);
- **isolation** (isolation from resources outside the network).

Any option that narrows VPC to a single function (just a firewall, just routing, just DNS) is wrong: VPC is the umbrella, not one of the primitives under it.

### What a new project gets by default
A new project is **automatically** provided with a set of default Virtual Networks resources (network, subnet, address pools, routing table) so you can launch a VM immediately without assembling networking by hand. The "nothing is created, build it all manually from scratch" option is a distractor.

### Security groups (packet firewall) vs routing
- **Security group** operates at the **packet-filtering** layer: rules (source/dest, ports, protocol, allow/deny, priority) attach to a NIC and filter ingress/egress. This is an *allow/deny* decision.
- **Routing table / routes** decide *where* a packet goes next. This is a *where-to-send* decision.

Distractor: a security-group definition that talks about "which subnets map to which routing tables" — that's the routing concept swapped in.

### Reasoning pattern
For a component, ask: does it make an *allow/deny* decision or a *where-does-it-go* decision? Firewall/security group = the former; routing table/routes = the latter. If the question is about a *service* (not a single component), the correct answer is usually broad: "provides the whole set of primitives + connectivity and isolation."

---

## 5. IAM privilege boundaries (create vs read-sensitive)

The key principle, which appeared twice: **the ability to create a resource does not imply the ability to read the sensitive secret/credential inside it.**

### Service-account keys
The `editor` role lets you create and configure resources (including creating a service account). But a service account's authorized keys are effectively long-lived credentials that let you authenticate *as* that identity. Handing out / minting credentials is an identity/security-tier operation, so it's gated behind a higher role (`admin`) even though `editor` can create the account itself.

### MysteryBox — reading the payload
Same logic for secrets: `editor` is enough to create a secret and a version, but **reading the payload** (the secret value itself) is a separate, more sensitive capability requiring a dedicated viewer/admin-tier role (⚠️ verify the exact role/sub-role name against the docs). The fact that you just created the secret deliberately does not grant the right to read its contents back.

### Traps
- Invented mechanisms: "encryption finalizes after 24 hours," "CLI vs web console," "must be added somewhere first."
- Blaming a permission error on a config bug / replication lag instead of a privilege-tier difference.

### Reasoning pattern
If a permission error appears **right after a successful create**, and the denied action touches **secrets, keys, or credentials**, look for a separate viewer/admin-tier permission — not a bug and not "propagation."

---

## 6. Billing model — commitment, reservation, PAYG

- **Commitment** — a signed spending agreement with a discount; the discount is applied via an **addendum** that governs how a resource *type* is charged. It is **not a hard cap** at exactly the committed quantity.
- Therefore, usage beyond the commitment for an **already-covered resource type** can still be charged at the commitment rate (the discount follows the resource class) rather than snapping back to full list price.
- A resource of a **type that was never part of the commitment** is billed at standard **PAYG** (pay-as-you-go).

### Traps
- "Overage is blocked / not billed." Commitments generally don't block overage — they set a discounted floor.
- "Nothing is billed until the contract is manually amended."
- Collapsing both resources into the same billing treatment.

### Reasoning pattern
Separate "more of a committed resource" from "a resource outside the commitment." A commitment sets a *discount mechanism*, not a *usage wall*. Overage of a covered type usually keeps the discount; anything uncovered defaults to PAYG.

---

## 7. GPU clusters and InfiniBand for distributed training

Multi-node distributed training performs well only over a high-bandwidth, low-latency fabric — that's the role of **InfiniBand**. On Nebius the construct for this is a **GPU cluster**:
- create the cluster with an InfiniBand fabric;
- place each VM into it via a cluster ID.

Both VMs referencing the same cluster ID are physically co-located on the same fabric, which is what enables fast collective traffic (all-reduce, etc.). An option that skips the GPU-cluster construct, or fails to bind the VMs to the same cluster ID, fails the interconnect requirement.

⚠️ Verify exact command/flag names (`gpu-cluster create --infiniband-fabric ...`, `--gpu-cluster-id ...`) against the CLI/Terraform documentation.
...
The conceptual fork:
- **Permissive policy** — the reservation is treated as a *preference*; if the reservation is unavailable it quietly spills over into on-demand (the common pool).
- **Strict policy** — the reservation is treated as a *hard requirement*; if the reserved capacity isn't there, the launch **fails loudly**, with no fallback to on-demand.

If the prompt says "under no circumstances fall back to on-demand; if the reservation is unavailable, the launch should fail" — that describes the strict behavior. Any policy whose semantics allow fallback is eliminated by the constraint itself.

⚠️ Verify the exact policy value name and flag spelling against the docs — the *behavior* ("fail rather than fall back") is what's tested, but the identifier should be confirmed, not guessed.

---

## 9. CLI profiles (working across multiple projects/regions)

Problem: working with two projects in **different regions**; you don't want to pass `--parent-id` and `--region` on every call, and there's a risk of accidentally creating a resource in the wrong region.

Solution — a **CLI profile**: it bundles the "who and where" context (tenant, project/parent, region) into a named unit. One profile per project:
- the region "travels" with the profile → you can't accidentally leak one project's region into the other's commands (with the correct profile selected);
- switch via the `--profile <name>` flag or a per-shell default.

### Traps
- Manually exporting / re-typing region/parent — reintroduces the human-error risk.
- Cramming both projects into a single profile — kills the isolation; you'd be back to overriding region per call.

### Reasoning pattern
If the prompt complains about repetitive context flags **plus** a cross-region "don't do the wrong thing by accident" risk — the answer is almost always "encode each context as a named profile and switch between them," not "remember to pass the right flags."

⚠️ Verify exact syntax (`nebius profile create ...`, setting a per-shell default, precedence of `--flag` over a profile value) against the CLI documentation.

---

## 10. User management via cloud-init

- The cloud-init config for users/SSH keys is **declarative and authoritative**, but by default it is applied only at first instance boot.
- If you change user-data on a running instance, nothing happens on its own — the new config must be **re-applied**.
- The users/SSH-keys block is a **full replacement, not a merge**: the updated user-data must include the original user *plus* the new ones, or you can lock out the original.

### Traps
- "Append one key" — doesn't fit the full-replacement model.
- Stop/start the VM — doesn't re-run cloud-init modules the way people often assume.

### Reasoning pattern
Update user-data to list all users (original + new), then re-run cloud-init on the running machine.

---

## 11. Reading `nvidia-smi`

Healthy signals in the snapshot we discussed: 100% utilization on all 8 GPUs, temperature in range, no throttling flags set.

- High memory occupancy (e.g. 78/80 GB) during training is **normal and often deliberate**: frameworks and their caching allocators (e.g. PyTorch) hold onto memory. A high static occupancy number does **not** mean "OOM is imminent."
- OOM happens when a specific allocation can't be satisfied; you'd see it as the job crashing with an OOM error, not as "2 GB left" in a snapshot.
- Utilization in `nvidia-smi` means "some kernel was running," not "the GPU was doing useful work efficiently."

### Key insight
When every hardware signal says "GPUs are healthy and fully loaded" but the work is still slow, the bottleneck is usually **not** GPU compute: interconnect/communication overhead in multi-GPU, data-loading stalls, suboptimal kernels, synchronization.

### Reasoning pattern
Don't let one scary-looking number hijack the read of an otherwise-healthy snapshot. "High utilization + slow step time" is a classic "look beyond the GPU" signal. Distinguish real thermal/power throttling (flags set) from healthy full load.

---

## 12. Instance metadata service (credential-free authentication)

Task: a startup script inside a VM needs the instance ID, region, and a short-lived IAM token (so the VM can call the API as its attached service account) — **without any pre-baked credentials**.

Solution — the **instance metadata service**:
- an endpoint reachable only from *inside* the VM, at a fixed internal address;
- the platform itself vouches for the caller by virtue of the request originating on that instance;
- the VM asks "who am I" (instance ID, region) and "give me a token to act as my service account" → gets a **short-lived** IAM token on demand, with no static key on disk.

### Traps
- `/etc/hostname` may contain an ID but can't produce a token or the region.
- A long-lived static key in a file (e.g. `/etc/nebius/key.json`) directly violates "no pre-baked credentials" — the exact anti-pattern the metadata service exists to eliminate.
- Anonymous IAM API calls can't work: identity has to come from somewhere.

### Reasoning pattern
When a workload needs to authenticate but you're forbidden from placing a secret on it, the answer is almost always a platform-provided metadata/identity endpoint that issues ephemeral credentials based on the workload's own identity. Static keys on disk are the thing to instinctively distrust.

---

## Summary of recurring reasoning patterns

1. **Map the operation to its tool/verb** (Slurm; reading `nvidia-smi`).
2. **Separate the layers/axes:** availability vs health vs billing (`drain`); filtering vs routing (VPC); connectivity vs isolation.
3. **Create ≠ read-secret:** rights over secrets/keys are a separate, higher tier (IAM, MysteryBox).
4. **A commitment is a discount mechanism, not a wall:** a covered type keeps the discount in overage; uncovered = PAYG.
5. **Preserve and reuse a construct** instead of recreating/copying it (IP allocation, CLI profiles, GPU cluster).
6. **Match on behavior, not on a scary number or an invented mechanism** (`nvidia-smi`; reservation policy; permission errors).
7. **Bootstrap with no secret → metadata service with ephemeral tokens.**

> ⚠️ The marked items (reservation-policy flag names, CLI profile syntax, GPU-cluster command names, the exact role for reading a MysteryBox payload) should be verified against the official Nebius documentation (CLI / Terraform / IAM) — the concepts here are reliable, but exact strings are better confirmed than approximated.
